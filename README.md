...
Narrative Canvas for Game Design 是一个面向游戏叙事设计的节点式画布。它适合用来拆分剧情段落、角色对白、选择分支、条件判断和变量设置，并把这些内容串成一条可以预览的互动流程。

这个仓库同时支持两种使用方式：

- **网页端**：直接打开 `index.html`，或访问 <https://ringeringeraja33.github.io/NarrativeCanvas4GameDesign/>。
- **Obsidian 插件端**：把仓库复制到 Obsidian vault 的 `.obsidian/plugins/narrative-canvas/`，在 Community plugins 中启用 `Narrative Canvas`。
...
- `main.js`：Obsidian 插件入口，把画布加载到 iframe
...
Narrative Canvas for Game Design is a node-based canvas for planning game narratives. It helps break a story into scenes, dialogue, choices, conditions, variable changes, and jumps, then connects them into a playable flow.
...
- Project variable editing as JSON
