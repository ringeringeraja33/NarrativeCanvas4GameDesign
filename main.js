...
module.exports = class NarrativeCanvasPlugin extends Plugin {
  async onload() {
    ...
    // before the canvas singleton is touched. We no longer need a workspace file-open hook
    ...
    this.registerEvent(this.app.workspace.on("active-leaf-change", (leaf) => {
      ...
      if (!file) return;
      if (this.settings.currentProjectPath === file) return;
      this.setCurrentProjectPath(file, { leaf, syncViewState: false })
  ...
  findProjectLeaf(path) {
    ...
    return leaves.find((leaf) => {
      const ownFile = leaf.view?.file;
      const state = leaf.getViewState?.()?.state || {};
      const leafPath = normalizeVaultPath(ownFile || state.file || state.path);
  ...
  async handleVaultFileDelete(file) {
    if (!file) return;
    const deletedPath = normalizeVaultPath(file.path);
    if (!deletedPath) return;
    const isFolder = file instanceof TFolder;
    const isProjectFile = file instanceof TFile && isProjectFileExtension(file.extension);
    if (!isFolder && !isProjectFile) return;
    const matchesDeleted = (candidate) => {
      const normalized = normalizeVaultPath(candidate);
      if (!normalized) return false;
      if (normalized === deletedPath) return true;
      return isFolder && normalized.startsWith(`${deletedPath}/`);
    };
    const matchingLeaves = (this.app.workspace?.getLeavesOfType?.(VIEW_TYPE) || []).filter((leaf) => {
      const state = leaf.getViewState?.()?.state || {};
      return matchesDeleted(leaf.view?.file || state.file || state.path);
    });
    const trackedCurrent = matchesDeleted(this.settings.currentProjectPath);
    const trackedLast = matchesDeleted(this.settings.lastProjectPath);
    if (!matchingLeaves.length && !trackedCurrent && !trackedLast) return;
    // Only stop the singleton canvas app if it is currently bound to the deleted file. Other
    // open Narrative Canvas tabs keep their state untouched.
    if (trackedCurrent) {
      try { window.NarrativeCanvasApp?.destroy?.(); } catch (error) { console.error(error); }
    }
    let changed = false;
    if (trackedCurrent) {
      this.settings.currentProjectPath = "";
    ...
    if (matchingLeaves.length) new Notice(`Narrative Canvas closed — ${file.path} was deleted.`);
  }

  async handleVaultFileRename(file, oldPath) {
    if (!(file instanceof TFile) || !isProjectFileExtension(file.extension)) return;
    const newPath = normalizeVaultPath(file.path);
    const previous = normalizeVaultPath(oldPath);
    if (!newPath || !previous || newPath === previous) return;
    let changed = false;
    if (normalizeVaultPath(this.settings.currentProjectPath) === previous) {
      this.settings.currentProjectPath = newPath;
      changed = true;
    }
    if (normalizeVaultPath(this.settings.lastProjectPath) === previous) {
      this.settings.lastProjectPath = newPath;
      changed = true;
    }
    if (changed) {
      await this.savePluginData();
    ...
    for (const leaf of leaves) {
      const viewState = leaf.getViewState?.();
      const leafPath = normalizeVaultPath(viewState?.state?.file || viewState?.state?.path);
      const ownFile = normalizeVaultPath(leaf.view?.file);
      if (leafPath !== previous && ownFile !== previous) continue;
      // Update the per-view tracker so the tab header re-renders to the new name.
      if (leaf.view) leaf.view.file = newPath;
      leaf.updateHeader?.();
      this.syncingProjectViewState = true;
      try {
        await leaf.setViewState({
          ...viewState,
          state: { ...(viewState.state || {}), file: newPath }
        });
      } finally {
        this.syncingProjectViewState = false;
      }
...
class NarrativeCanvasView extends ItemView {
  constructor(leaf, plugin) {
    ...
    this.plugin = plugin;
    this.styleEl = null;
    // Each leaf tracks the file it is showing. Without this, getDisplayText / getState fell
    // back to the plugin-level singleton (last-opened file), so multiple open .ncanvas tabs
    // all showed the same name in their tab header.
    this.file = "";
    // Set true the moment we detect this leaf is a duplicate. Both setState and onOpen
    // bail early when this is set so we never install a singleton on a doomed leaf.
    this.duplicateOf = null;
    // Earliest possible dedup: read the leaf's already-stored state inside the constructor
    // and schedule self-detach if a sibling NC leaf already shows the same file.
    this.tryDedupNow();
  }
  ...
  getDisplayText() {
    return projectDisplayName(this.file)
      || projectDisplayName(this.plugin.getCurrentProjectPath())
      || "Narrative Canvas";
  }
  ...
  getState() {
    ...
    return this.file ? { ...state, file: this.file } : state;
  }

  // Look at every NC leaf except mine. If any of them already shows the same intended file
  // (either via its tracked view.file or the leaf's stored state), schedule a self-detach.
  // Called from constructor, setState, and onOpen; safe to call any number of times.
  tryDedupNow() {
    if (this.duplicateOf) return true;
    const intendedFile = this.file
      || normalizeVaultPath(this.leaf?.getViewState?.()?.state?.file)
      || normalizeVaultPath(this.leaf?.getViewState?.()?.state?.path);
    if (!intendedFile) return false;
    const workspace = this.app?.workspace;
    const allLeaves = workspace?.getLeavesOfType?.(VIEW_TYPE) || [];
    const survivor = allLeaves.find((leaf) => {
      if (leaf === this.leaf) return false;
      const otherFile = normalizeVaultPath(
        leaf.view?.file
        || leaf.getViewState?.()?.state?.file
        || leaf.getViewState?.()?.state?.path
      );
      return otherFile && otherFile === intendedFile;
    });
    if (!survivor) return false;
    this.duplicateOf = survivor;
    workspace?.setActiveLeaf?.(survivor, { focus: true });
    workspace?.revealLeaf?.(survivor);
    // Defer to the next microtask so we don't tear ourselves down inside our own
    // constructor / setState / onOpen call frame.
  ...
  async setState(state, result) {
    ...
    const fileChanged = this.file !== file;
    this.file = file;
...
const FolderSuggestBase = typeof AbstractInputSuggest === "function" ? AbstractInputSuggest : null;

class FolderSuggest extends (FolderSuggestBase || class {}) {
  constructor(app, inputEl) {
    if (!FolderSuggestBase) return;
    super(app, inputEl);
    this.app = app;
  ...
  getSuggestions(query) {
    ...
    this.app.vault.getAllLoadedFiles().forEach((file) => {
      if (file instanceof TFolder && file.path.toLowerCase().includes(normalized)) {
        folders.push(file);
      }
    });
  ...
  display() {
    ...
    new Setting(containerEl)
      ...
      .addText((textInput) => {
        ...
        if (FolderSuggestBase) {
          try { new FolderSuggest(this.app, textInput.inputEl); }
          catch (_error) { /* AbstractInputSuggest unavailable */ }
        }
      });
...
const CANVAS_INDEX_HTML = [
  ...
  "    \u003clink rel=\"stylesheet\" href=\"./canvas.css?v=1.1.0\"\u003e",
  "  \u003c/head\u003e",
  "  \u003cbody\u003e",
  "    \u003cdiv class=\"app-shell\"\u003e",
  ...
  "            \u003c/label\u003e",
  "            \u003clabel class=\"workspace-search-box playbook-search-box\" data-search-scope=\"variables\" hidden\u003e",
  "              Find",
  ...
  "          \u003cbutton id=\"genericConfirmSecondaryButton\" class=\"small-button\" value=\"secondary\" hidden\u003eKeep\u003c/button\u003e",
  "          \u003cbutton class=\"small-button confirm-cancel\" value=\"cancel\" autofocus\u003eCancel\u003c/button\u003e",
  "        \u003c/footer\u003e",
  "      \u003c/form\u003e",
  "    \u003c/dialog\u003e",
  ...
  "            \u003ch3\u003eCan do\u003c/h3\u003e",
  "            \u003cul\u003e",
  "              \u003cli\u003eStore project variables for text such as \u003ccode\u003e{traveler}\u003c/code\u003e.\u003c/li\u003e",
  "              \u003cli\u003eBatch-edit node Requirements, Effects, and Routing in Script Builder.\u003c/li\u003e",
  "              \u003cli\u003eGate simple branches with conditions such as \u003ccode\u003eflag == true\u003c/code\u003e.\u003c/li\u003e",
  "              \u003cli\u003eControl the demo runner with Start Node, Choice Display, End Condition, Visit Tracking, and Debug Mode.\u003c/li\u003e",
  "              \u003cli\u003eUse Advanced JSON for exact edits without changing the canvas schema.\u003c/li\u003e",
  "            \u003c/ul\u003e",
  "          \u003c/section\u003e",
  "          \u003csection\u003e",
  "            \u003ch3\u003eCannot do\u003c/h3\u003e",
  ...
  "              \u003cli\u003eReplace a game engine runtime or evaluate complex code expressions.\u003c/li\u003e",
  "            \u003c/ul\u003e",
  "          \u003c/section\u003e",
  "        \u003c/div\u003e",
  "  \u003cpre class=\"playbook-example\"\u003e\u003ccode\u003e{",
  "  \"variables\": { \"has_badge\": false, \"trust\": 1 },",
  "  \"playRules\": {",
  ...
  "          \u003cbutton type=\"button\" data-action=\"create-play-rule\" data-playbook-rule-kind=\"startNode\"\u003e",
  "            \u003cstrong\u003eStart Node\u003c/strong\u003e",
  "            \u003cspan\u003eChoose the exact node title Play starts from.\u003c/span\u003e",
  "          \u003c/button\u003e",
  "          \u003cbutton type=\"button\" data-action=\"create-play-rule\" data-playbook-rule-kind=\"choiceDisplay\"\u003e",
  "            \u003cstrong\u003eChoice Display\u003c/strong\u003e",
  "            \u003cspan\u003eDecide how gated choices appear in Play.\u003c/span\u003e",
  "          \u003c/button\u003e",
  "          \u003cbutton type=\"button\" data-action=\"create-play-rule\" data-playbook-rule-kind=\"endCondition\"\u003e",
  "            \u003cstrong\u003eEnd Condition\u003c/strong\u003e",
  "            \u003cspan\u003eStop the demo route when a condition becomes true.\u003c/span\u003e",
  "          \u003c/button\u003e",
  "          \u003cbutton type=\"button\" data-action=\"create-play-rule\" data-playbook-rule-kind=\"visitTracking\"\u003e",
  "            \u003cstrong\u003eVisit Tracking\u003c/strong\u003e",
  "            \u003cspan\u003eRecord visited node titles during Play.\u003c/span\u003e",
  "          \u003c/button\u003e",
  "          \u003cbutton type=\"button\" data-action=\"create-play-rule\" data-playbook-rule-kind=\"debugMode\"\u003e",
  "            \u003cstrong\u003eDebug Mode\u003c/strong\u003e",
  "            \u003cspan\u003eShow variable and condition details while previewing.\u003c/span\u003e",
  "          \u003c/button\u003e",
  "        \u003c/div\u003e",
  "        \u003cp class=\"playbook-rule-allset\" data-playbook-rule-allset hidden\u003eAll play rules already enabled.\u003c/p\u003e",
  "        \u003cfooter class=\"confirm-actions\"\u003e",
  "          \u003cbutton class=\"small-button confirm-cancel\" value=\"cancel\" autofocus\u003eCancel\u003c/button\u003e",
  "        \u003c/footer\u003e",
  "      \u003c/form\u003e",
  ...
  "          \u003c/label\u003e",
  "          \u003clabel class=\"nc-checkbox-field\"\u003e",
  "            \u003cinput id=\"nodeTypeEventHiddenInput\" type=\"checkbox\"\u003e",
  "            \u003cspan\u003eHide frame rows from Events Sheet\u003c/span\u003e",
  "          \u003c/label\u003e",
  "        \u003c/div\u003e",
  "        \u003cfooter class=\"confirm-actions\"\u003e",
